...
assert not any(point["number"] in {1, 2, 84, 85} for point in points)
required_low_frequency = {28, 48, 49, 51, 70, 89}
assert required_low_frequency <= {point["number"] for point in points}, "Low-frequency examinable topics must be retained"
...
assert 1919 in next(point for point in points if point["number"] == 14)["relatedYears"]
for keyword in ("鸦片战争", "五四运动", "九一八事变", "卢沟桥事变", "全面内战", "中共八大", "中华人民共和国"):
